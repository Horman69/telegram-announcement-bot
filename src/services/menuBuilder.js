...
class MenuBuilder {
    ...
    getMainMenuText(userName, isAdmin) {
        ...
        text += `Я бот для рассылки объявлений в группы.\n\n`;
    ...
    getAdminManagementMenu() {
        try {
            const buttons = [
                [Markup.button.callback('➕ Добавить администратора', 'menu:action:add_admin')],
                [Markup.button.callback('📋 Список администраторов', 'menu:action:list_admins')],
                [Markup.button.callback('◀️ Назад', 'menu:admin')]
            ];
