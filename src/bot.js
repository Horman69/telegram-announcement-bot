...
function initializeGroupHandlers() {
    ...
    bot.on('my_chat_member', (ctx) => {
        ...
        if ((newStatus === 'member' || newStatus === 'administrator') &&
            ...
            ctx.reply(
                `👋 Привет! Я бот для рассылки объявлений.\n\n` +
                `Эта группа добавлена в список для рассылки.\n` +
                `Администраторы могут отправлять объявления командой /announce`
            );
        }
...
function initializeTextIgnore() {
    ...
    bot.on('text', (ctx) => {
        // Просто игнорируем сообщение, ничего не делаем
